<?php 
include '../config/koneksi.php';
session_start();

// 1. Cek keamanan akses
if ($_SESSION['role'] != "admin") {
    header("location:../login.php");
    exit();
}

// 2. Ambil data dari URL
$id     = mysqli_real_escape_string($koneksi, $_GET['id'] ?? '');
$status = mysqli_real_escape_string($koneksi, $_GET['status'] ?? '');

if (empty($id) || empty($status)) {
    echo "ID atau Status tidak valid.";
    exit();
}

// 3. Logika Update Database Berdasarkan Status
if ($status == 'disetujui') {
    // Pastikan ruangan_id ada jika disetujui
    if (!isset($_GET['ruangan_id']) || empty($_GET['ruangan_id'])) {
        echo "<script>alert('ID Ruangan tidak terpilih!'); window.history.back();</script>";
        exit();
    }
    $ruangan_id = mysqli_real_escape_string($koneksi, $_GET['ruangan_id']);
    $query = "UPDATE reservasi SET status = 'disetujui', ruangan_id = '$ruangan_id' WHERE id = '$id'";

} elseif ($status == 'dibatalkan') {
    // Ambil alasan pembatalan dari modal
    $alasan = mysqli_real_escape_string($koneksi, $_GET['alasan'] ?? 'Alasan operasional mendadak.');
    $query = "UPDATE reservasi SET status = 'dibatalkan', catatan_admin = '$alasan' WHERE id = '$id'";

} else {
    // Status Ditolak
    $query = "UPDATE reservasi SET status = 'ditolak' WHERE id = '$id'";
}

// Eksekusi Update
$eksekusi = mysqli_query($koneksi, $query);

if ($eksekusi) {
    // 4. Ambil data lengkap untuk notifikasi WA (Gunakan LEFT JOIN)
    $q_info = mysqli_query($koneksi, "SELECT r.*, u.nama_lengkap, u.no_wa, rm.nama_ruangan 
                                      FROM reservasi r 
                                      JOIN users u ON r.user_id = u.id 
                                      LEFT JOIN ruangan rm ON r.ruangan_id = rm.id 
                                      WHERE r.id = '$id'");
    $d = mysqli_fetch_array($q_info);

    // 5. Normalisasi nomor WA User
    $phone = preg_replace('/[^0-9]/', '', $d['no_wa'] ?? '');
    if (!empty($phone)) {
        if (substr($phone, 0, 1) === '0') { $phone = '62' . substr($phone, 1); }
        elseif (substr($phone, 0, 1) === '8') { $phone = '62' . $phone; }
    }

    // 6. Susun Pesan WA Dinamis
    $nama_sistem = $sett['nama_sistem'] ?? 'PINTU WKP';
    $waktu = date('d/m/Y', strtotime($d['tgl_pinjam']));
    if ($d['tipe_permintaan'] == 'meeting_room') {
        $waktu .= " (" . substr($d['jam_mulai'], 0, 5) . " - " . substr($d['jam_selesai'], 0, 5) . " WITA)";
    }
    
    $pesan = "Assalamualaikum, Akh *" . $d['nama_lengkap'] . "*,\n\n";

    if ($status == 'disetujui') {
        $pesan .= "Kabar baik! Pengajuan ruangan Anda di *" . $nama_sistem . "* telah *DISETUJUI*.\n\n";
        $pesan .= "*Detail Alokasi:* \n";
        $pesan .= "• Ruangan: " . ($d['nama_ruangan'] ?? '-') . "\n";
    } elseif ($status == 'dibatalkan') {
        $pesan .= "Mohon maaf, pengajuan ruangan Anda di *" . $nama_sistem . "* terpaksa kami *BATALKAN*.\n\n";
        $pesan .= "*Alasan:* " . ($_GET['alasan'] ?? 'Alasan operasional mendadak.') . "\n\n";
        $pesan .= "*Detail:* \n";
        $pesan .= "• Ruangan: " . ($d['nama_ruangan'] ?? '-') . "\n";
    } else {
        $pesan .= "Mohon maaf, pengajuan ruangan Anda di *" . $nama_sistem . "* saat ini *DITOLAK* / Belum dapat disetujui.\n\n";
        $pesan .= "*Detail:* \n";
        $pesan .= "• Layanan: " . strtoupper(str_replace('_', ' ', $d['tipe_permintaan'])) . "\n";
    }

    $pesan .= "• Waktu: " . $waktu . "\n\n";

    if ($status == 'disetujui') {
        $pesan .= "Silakan gunakan ruangan sesuai jadwal. Terima kasih.";
    } else {
        $pesan .= "Silakan hubungi Admin Gedung untuk informasi lebih lanjut. Terima kasih.";
    }

    $url_wa = "https://wa.me/" . $phone . "?text=" . urlencode($pesan);

    // 7. TAMPILKAN UI SUKSES
    $color = ($status == 'disetujui') ? 'success' : 'danger';
    $icon  = ($status == 'disetujui') ? 'bi-check-circle-fill' : 'bi-x-circle-fill';
    $title = ($status == 'disetujui') ? 'Berhasil Disetujui' : ($status == 'dibatalkan' ? 'Berhasil Dibatalkan' : 'Berhasil Ditolak');
?>

    <!DOCTYPE html>
    <html lang="id">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Status Diperbarui</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css">
        <style>
            body { font-family: 'Poppins', sans-serif; background: #1e293b; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; }
            .status-card { background: white; padding: 30px; border-radius: 25px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.5); text-align: center; max-width: 450px; width: 90%; }
        </style>
    </head>
    <body>
        <div class="status-card">
            <div class="mb-3 text-<?php echo $color; ?>">
                <i class="bi <?php echo $icon; ?>" style="font-size: 60px;"></i>
            </div>
            <h4 class="fw-bold"><?php echo $title; ?></h4>
            <p class="text-muted small">Status pengajuan telah diperbarui. Silakan klik tombol di bawah untuk mengabari User via WhatsApp.</p>
            <a href="<?php echo $url_wa; ?>" target="_blank" class="btn btn-<?php echo $color; ?> w-100 py-3 mb-2 fw-bold" style="border-radius: 15px;">
                <i class="bi bi-whatsapp me-2"></i> Kabari User via WA
            </a>
            <a href="persetujuan.php" class="btn btn-link text-muted text-decoration-none small">Kembali ke Daftar</a>
        </div>
    </body>
    </html>
<?php
    exit();
} else {
    echo "Error Database: " . mysqli_error($koneksi);
}
?>
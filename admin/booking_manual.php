...
<head>
    ...
    <style>
        ...
        .header-section { background: #1e293b; color: white; padding: 30px 20px 50px; border-radius: 0 0 30px 30px; margin-bottom: -30px; }
        ...
        #sug-box { display: none; position: absolute; width: 100%; background: white; z-index: 1001; border-radius: 15px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); border: 1px solid #eee; top: 100%; margin-top: 5px; }
        ...
        <div class="card form-card p-4">
            <form action="booking_manual_aksi.php" method="post">
                ...
                <div class="mb-3 position-relative">
                    ...
                    <div id="sug-box"><div class="list-group list-group-flush">
                        <?php while($i = mysqli_fetch_array($query_inst)) { echo '<div class="list-group-item sug-item">'.$i['institusi_peminjam'].'</div>'; } ?>
                    </div></div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            input.on("keyup focus", function() {
                ...
                if (val.length === 0) { box.hide(); return; }
                var match = 0;
                $(".sug-item").each(function() {
                    if ($(this).text().toLowerCase().indexOf(val) > -1) { $(this).show(); match++; }
                    else { $(this).hide(); }
                });
                if (match > 0) box.show(); else box.hide();
            });
            $(document).on("click", ".sug-item", function() { input.val($(this).text()); box.hide(); });
        });
    </script>
</body>
</html>